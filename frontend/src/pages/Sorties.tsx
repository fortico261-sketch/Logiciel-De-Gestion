...
import { addStockExit } from '../redux/inventorySlice'
import { getTenantId } from '../redux/authSlice'
import { formatMGA } from '../utils/formatCurrency'
import { motion } from 'framer-motion'
import {
  ArrowUpFromLine, Package, Store, Search, Filter,
  Plus, User, AlertTriangle,
...
type ExitReason =  'perte' | 'ajustement' | 'retour_fournisseur'

const reasonLabels: Record<ExitReason, string> = {
  
  perte: 'Perte / Casse',
  ajustement: 'Ajustement négatif',
  retour_fournisseur: 'Retour fournisseur'
}

const reasonColors: Record<ExitReason, string> = {
  perte: 'bg-red-50 text-red-600 border-red-200',
  ajustement: 'bg-amber-50 text-amber-600 border-amber-200',
  retour_fournisseur: 'bg-orange-50 text-orange-600 border-orange-200'
}
...
export const Sorties: React.FC = () => {
  ...
  // Filtrer les mouvements de sortie
  const sorties = useMemo(() => {
    let filtered = movements.filter(m => m.type === 'sortie')
  ...
  // Statistiques
  const stats = useMemo(() => {
    const totalExits = sorties.reduce((sum, m) => sum + m.quantity, 0)
    const totalValue = sorties.reduce((sum, m) => {
      const product = products.find(p => p.id === m.productId)
      return sum + (product?.price || 0) * m.quantity
    }, 0)
    const byReason = sorties.reduce((acc, m) => {
      acc[m.reason] = (acc[m.reason] || 0) + m.quantity
      return acc
    }, {} as Record<string, number>)
    
    return { totalExits, totalValue, byReason }
  }, [sorties, products])
  ...
  const handleSubmit = async () => {
    ...
    const currentStock = product?.stockByPoint[selectedPoint] || 0
    if (currentStock < quantity) {
      toast.error(`Stock insuffisant (${currentStock} disponible${currentStock > 1 ? 's' : ''})`)
      return
    }
    ...
    <section className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 font-display flex items-center gap-3">
            <ArrowUpFromLine className="w-8 h-8 text-danger-500" />
            Sorties de stock
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Suivi des sorties : ventes, pertes, retours fournisseurs et ajustements.
          </p>
        </div>
        <Button 
          variant="primary" 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-semibold text-slate-400 uppercase">Total sorties</div>
              <div className="text-2xl font-extrabold text-slate-900">{stats.totalExits} unités</div>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-2xl font-extrabold text-slate-900">{sorties.length}</div>
            </div>
          </div>
        </motion.div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200/60 bg-white p-5 shadow-soft">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <select
            value={filterReason}
            onChange={(e) => setFilterReason(e.target.value as ExitReason | 'all')}
            className="h-11 px-4 rounded-xl border border-slate-200 bg-white text-sm font-semibold outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-100 min-w-[160px]"
          >
                ...
                sorties.map((movement) => (
                  <tr key={movement.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="inline-flex items-center justify-center px-3 py-1 rounded-xl text-xs font-bold bg-danger-100 text-danger-700">
                        -{movement.quantity}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold border ${reasonColors[movement.reason as ExitReason] || 'bg-slate-100 text-slate-600'}`}>
                        {reasonLabels[movement.reason as ExitReason] || movement.reason}
                      </span>
                    </td>
      ...
      <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)} title="Nouvelle sortie de stock">
        <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="space-y-5">
          <div className="p-3 bg-amber-50 border border-amber-100 rounded-2xl flex items-start gap-3">
            <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
            <div className="text-xs text-amber-700">
              <span className="font-bold">Attention :</span> Une sortie de stock réduit définitivement le stock disponible. 
              Vérifiez bien la quantité avant de valider.
            </div>
          </div>
          ...
          <div>
            ...
            >
              ...
              {products.map((p) => {
                const stock = selectedPoint ? (p.stockByPoint[selectedPoint] || 0) : 0
                return (
                  <option key={p.id} value={p.id}>
                    {p.name} ({p.sku}) - Stock: {stock} u
                  </option>
                )
              })}
            </select>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                required
              />
              {selectedProduct && selectedPoint && (
                <div className="text-xs text-slate-400 mt-1">
                  Stock disponible: {products.find(p => p.id === selectedProduct)?.stockByPoint[selectedPoint] || 0} unités
                </div>
              )}
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Motif de la sortie</label>
            <select
              value={reason}
